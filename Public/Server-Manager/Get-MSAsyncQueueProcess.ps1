Function Get-MSAsyncQueueProcess {
    ...
    process {
        ...
        Set-MSTimestamp;
        $Request = ("<IODATA loginguid='[!guid_login!]' sessionkey='[!key!]'><ADMINISTRATION><ASYNCQUEUE action='load' guid='[!guid_process!]'/></ADMINISTRATION></IODATA>").Replace("[!guid_process!]", ($ProcessGUID|ConvertTo-RQLGuid));
